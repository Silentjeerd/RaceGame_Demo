...
public class CheckpointManager : MonoBehaviour
{
    ...
    GUIStyle textStyle; //Textstyle to be used.
    ...
    private Checkpoint[] checkpointsArray; //Array of all checkpoints on the track.

    //Start is called before the first frame update
    void Awake()
    {
        Instance = this;
        textStyle = GameManager.Instance.getTextStyle(); //Gets the textStyle from the gamemanager.
        lapCount = 1;//Sets lapCount to the first lap.
        checkpointsArray = GetComponentsInChildren<Checkpoint>(); //Fills the array with all child objects that have the checkpoint script attached to them.
    }
